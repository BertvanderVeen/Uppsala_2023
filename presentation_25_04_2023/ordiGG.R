...
#'
#' @param ord_in input ordination ord_in
#' @param grp_in vector of grouping ord_ins for the biplot, must have the same number of observations as the original matrix used for the ordination
...
#' @import ggplot2 plyr
#'
#' @return A \code{\link[ggplot2]{ggplot}} ord_in that can be further modified
#'
...
#'
#' # build a tree and phylo4d ord_in
...
#' tdata(liz.4d, type="tip") <- newdat[,-1] # replace data in the phylo4d ord_in
#'
#' # create ppca
...
ggord.gllvm <- function(ord_in, type = NULL, grp_in = NULL, axes = c(1,2), rotate = FALSE,...){
  p <- ncol(ord_in$y)
  ...
  choose.lvs <- lv
  if(quadratic == FALSE){choose.lv.coefs <- ord_in$params$theta}else{choose.lv.coefs<-gllvm::optima(ord_in,sd.errors=F)}  
  ...
  scaled_cw_sites <- t(t(choose.lvs) / sqrt(colSums(choose.lvs^2)) * (bothnorms^0.5)) 
  ...
  for(i in 1:ncol(scaled_cw_species)){
    scaled_cw_species[,i] <- choose.lv.coefs[,i] / sqrt(sum(choose.lv.coefs[idx[,i],i]^2)) * (bothnorms[i]^(1-0.5)) 
  }
